...
enum TransactionState { Sucess, Failure }
...
var logged_user: LoginState;
const balances = new Map<UserName, number>();
const database: UserRegister[] = []
...
function general_menu() {
    ...
    while(inloop) {
        let welcome = prompt(
            "¡Bienvenido/a a tu cajero automático móvil! \n" +
        ...
        switch (welcome) {
            case '1':
                ...
                if(logged_user !== undefined) logged_user_menu()
                break
            case '2': 
                signUpUser() 
                break
...
function logged_user_menu() {
    ...
    while(inloop) {
        let welcome = prompt(
            `¡Bienvenido/a a tu cuenta, ${logged_user}! \n` +
            "Por favor, escoge las siguientes opciones para continuar: \n" +
            "1. Mostrar balance. \n" +
            "2. Realizar retiro. \n" +
            "3. Realizar depósito.\n" +
        ...
        switch (welcome) {
            case '1': return_balance(); break
            case '2': transaction_menu(withdraw); break
            case '3': transaction_menu(deposit); break
            case '4': inloop = false
...
function signUpUser() {
    ...
    if (userAlreadyExists(user_name)) { alert("Usuario ya existente"); return }
    let user_email = prompt("Por favor, ingresa tu correo") || ""
...
// Validations
function userAlreadyExists(user: UserName): boolean {
...
function isRegisteredInDataBase(register: UserRegister) {
    for (let existing_register of database) {
        ...
        if (correct_user && correct_password) { return true }
    }
    return false
}
...
function return_balance() {
    if (logged_user === undefined) { throw Error("Usuario no está loggeado.") }
    alert(`${logged_user}, su balance es de ${balances.get(logged_user)}.`)
}

function withdraw(amount: number): TransactionState {
    if (amount < 0) { throw Error("Sólo es posible retirar si tiene valores positivos.") }
    if (logged_user === undefined) { throw Error("Usuario no está loggeado.") }
